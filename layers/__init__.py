...
def make_loss(cfg, num_classes):    # modified by gu
    ...
    if sampler == 'softmax':
        ...
        def loss_func(score, feat, target):
            if cfg.MODEL.METRIC_LOSS_TYPE == 'triplet':  # √
                if cfg.MODEL.IF_LABELSMOOTH == 'on':  # √
                    cls_loss = xent(score, target)  # 平滑交叉熵，分类损失 score shape[bs,751],target shape[bs],target为真实标签
                    tr_loss = triplet(feat, target)  # feat shape为[bs,2048]，target shape为[bs]，target为真实标签
